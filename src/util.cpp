...
const char * const BITCOIN_CONF_FILENAME = "edgy.conf";
const char * const BITCOIN_PID_FILENAME = "edgyd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "edgy" is a composite category enabling all EDGY-related debug output
            if(ptrCategory->count(string("edgy"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EDGYCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EDGYCore
    // Mac: ~/Library/Application Support/EDGYCore
    // Unix: ~/.edgycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EDGYCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EDGYCore";
#else
    // Unix
    return pathRet / ".edgycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty edgy.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override edgy.conf
